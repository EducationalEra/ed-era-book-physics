...
<ul>
<li>Знайти результуючу швидкiсть Петра i помножити на час $$t$$.</li>
    \[\vec{v}_{п/з} = \vec{v}_{п/р} + \vec{v}_{\text{р/з}} \Rightarrow v_{п/з} = \sqrt{{{v}_{п/р}}}^2 + {{v}_{\text{р/з}}}^2} = \sqrt{3^2 + 4^2} = 5 \thinspace (\text{м/с})\]
